...
# Nama file yang langsung dibaca oleh sistem
NAMA_FILE = "data_pelanggan (2).csv"

@st.cache_data
def load_data_otomatis():
    # Membaca file lokal dengan pemisah semikolon (;) sesuai struktur asli data kamu
    df = pd.read_csv(
        NAMA_FILE, 
        sep=";", 
...
try:
    # Mengambil data langsung tanpa menunggu upload
    df = load_data_otomatis()
    
    st.success(f"📊 Berhasil Memuat Data Otomatis: {len(df)} Pelanggan Terdaftar dari file '{NAMA_FILE}'.")
    ...
    # 6. Tata Letak Menu Menggunakan Tab
    ...
    st.error(f"❌ File **'{NAMA_FILE}'** tidak terdeteksi di folder proyekmu!")
    st.info(f"💡 Solusi: Pastikan file bernama **`data_pelanggan (2).csv`** sudah kamu pindahkan ke dalam folder yang sama dengan file script python (`data pelanggan.app.py`) kamu.")
